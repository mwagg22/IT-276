...
void check_tile_ahead(Entity* self, int** tiles){
	...
	int t = 0;
	for (int i = left_bound; i <= right_bound; i++){
		if (tiles[bottom_tile][i] != 19 && tiles[bottom_tile][i] != 10 && vector2d_magnitude_between(vector2d(0, self->position.y + self->hitbox.offsety + self->hitbox.h), vector2d(0, bottom_tile * 16)) <= 1.5){
			self->is_grounded = true;
			t++;
			if (self->type == ES_Player){
				if (tiles[bottom_tile][i] == 17)
					self->onDeath(self);
			}
		}
	}
	...
	int u = 0;
	for (int i = left_bound; i <= right_bound; i++){
		if (tiles[upper_tile][i] != 19 && tiles[upper_tile][i] != 10 && vector2d_magnitude_between(vector2d(0, self->position.y + self->hitbox.offsety), vector2d(0, upper_tile * 16 + 16)) <= 2.5){
	...
	for (int i = upper_bound; i <= lower_bound; i++){
		if (self->is_grounded&&i == lower_bound)
			continue;
		if (tiles[i][left_tile] != 19 && tiles[i][left_tile] != 10 && vector2d_magnitude_between(vector2d(self->position.x + self->hitbox.offsetx, 0), vector2d(left_tile * 16 + 16, 0)) <= 1){
			self->l_wall_collision = true;
			l++;
			//slog("left wall collision");
			if (self->type == ES_Player){
				if (tiles[i][left_tile] == 17)
					self->onDeath(self);
			}
		}
	}
	...
	for (int i = upper_bound; i <= lower_bound; i++){
		if (self->is_grounded&&i == lower_bound)
			continue;
		if (tiles[i][right_tile] != 19 && tiles[i][right_tile] != 10 && vector2d_magnitude_between(vector2d(self->position.x + self->hitbox.w + self->hitbox.offsetx, 0), vector2d(right_tile * 16, 0)) <= 1){
			self->r_wall_collision = true;
			//return;
			r++;
			//slog("right wall collision");
			if (self->type == ES_Player){
				if (tiles[i][right_tile] == 17)
					self->onDeath(self);
			}
...
bool check_empty_tile(Entity *self, int**tiles){
	...
	for (int i = left_bound; i <= left_bound; i++){
		if (tiles[bottom_tile][i] == 19 && vector2d_magnitude_between(vector2d(0, self->position.y + self->hitbox.offsety + self->hitbox.h), vector2d(0, bottom_tile * 16)) <= 1.5){
			return true;
		}
	}
