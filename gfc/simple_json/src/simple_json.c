...
SJson *sj_load(const char *filename)
{
    ...
    if (!file)
    {
        slog("sj_load: failed to open file");
        return NULL;
    }
    size = get_file_Size(file);
    if (size <= 0)
    {
        slog("sj_load: error with file size");
        fclose(file);
        return NULL;
    }
    ...
    if (buffer == NULL)
    {
        slog("sj_load: failed to allocate character buffer for json file");
        fclose(file);
        return NULL;
    }
...
SJson *sj_new()
{
    ...
    if (!json)
    {
        slog("sj_new: failed to allocate space for new json object");
        return NULL;
    }
...
int* sj_array_return(SJson *array){
	int size = sj_array_get_count(array);
	int val=0;
	int* output = (int *)malloc(size* sizeof(int));
	SJson *value;
	int ctr = 0;
	for (int i = 0; i < size; i++){
		value = sj_array_get_nth(array, ctr);
